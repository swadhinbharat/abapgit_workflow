CLASS zcl_generate_travel_data_sp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_travel_data_sp IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA:itab TYPE TABLE OF ztravel_sp.
...
*   delete existing entries in the database table
    DELETE FROM ztravel_sp.

*   insert the new table entries
    INSERT ztravel_sp FROM TABLE @itab.

*   check the result
    SELECT * FROM ztravel_sp INTO TABLE @itab.
    out->write( sy-dbcnt ).
